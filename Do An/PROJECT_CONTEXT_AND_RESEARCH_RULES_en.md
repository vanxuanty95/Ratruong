...
| V-003 | Exact Amazon Reviews'23 release, category, and post-filter counts for Phase 2 | `PARTIAL: PERSISTENT PROTOCOL AUDIT EXECUTED FOR BABY_PRODUCTS (2026-09-02); POST-FILTER (FROZEN) COUNT STILL MISSING BECAUSE THE PRIMARY POLICY IS NOT YET CHOSEN` | Pre-registered primary interaction policy (P4/P5/all-observed), duplicate/`0.0` treatment, and a training-only temporal graph built under the chosen policy | Select the primary policy from the evidence in `00_project/DATASET_PORTFOLIO_AND_ANALYSIS_PROTOCOL_en.md` Section 2.1, close G2-B, then build the frozen training graph to close G2-C |
...
- The cumulative thesis deliverables are initialized: bilingual working thesis reports under `04_thesis`, bilingual PowerPoint decks under `05_slides`, and bilingual-documented Python/Colab source under `06_code`. The local suite passes 13/13 pure-Python tests, including the G2-C toy path; this is not validation of the full Baby run, a final Phase 2 method, full oracle suite, or benchmark.
...
- The persistent portfolio audit confirms 693,929 valid `All_Beauty` rows and 5,953,891 parsed `Baby_Products` rows, zero exact duplicate pairs in both, and one quarantined Baby `rating=0.0`. Baby P4 and G2-A/G2-B are selected/passed; the provider absolute split is not the primary warm-start protocol.
- The portfolio analyzer and paired G2-C/G2-D notebooks now cover provenance through deterministic training-only graph construction, OOV reconciliation, exact-candidate rules, and bounded traversal. The full Baby G2-C/G2-D execution and evidence review remain open.
...
### Self-contained Colab dataset audit — 2026-09-02

- **Decision or result:** Replaced the external-script Colab launcher with a self-contained dataset-audit notebook. Each paired notebook now embeds the complete standard-library/SQLite analyzer, reuses or downloads the persistent raw artifact, runs the full protocol audit, and writes the persistent JSON without requiring a separate `.py` file on Drive.
- **Evidence/source:** Static parsing of every code cell in both notebooks; notebook JSON validation; embedded logic hash and notebook revision written into the result provenance; existing audit requirements in `06_code/docs/DATASET_AUDIT_en.md` and its Vietnamese counterpart.
- **Agents consulted, roles, and identifiers:** None. This was a mechanical packaging and execution-path correction requested directly by the user; it did not add a research decision or scientific interpretation.
- **Claims added, verified, contradicted, or retired:** Verified only that the local notebook artifacts are self-contained and syntactically valid. No full `Baby_Products` protocol run, model result, gate closure, or dataset-policy decision was produced by this change.
- **What remains uncertain:** The complete persistent Colab run outcome and the subsequent P4/P5/all-observed policy adjudication remain open.
- **Next action:** Run all cells in the Vietnamese or English Drive notebook once, then read the resulting `Baby_Products_protocol_audit.json` and integrate the evidence into the living thesis report before closing any G2 sub-gate.
- **Files created or changed in Do An:** both `01_amazon_dataset_audit_*` notebooks; both dataset-audit protocol documents; both code READMEs; both continuity files.


### Integrated persistent protocol-audit evidence for Baby_Products — 2026-09-02

- **Decision or result:** Accessed the user's private Google Drive (on their direct instruction), extracted the real `Baby_Products_protocol_audit.json` (folder `audit_outputs`, `retrieved_at` 2026-09-02T11:49:02Z), and integrated it into `00_project/DATASET_PORTFOLIO_AND_ANALYSIS_PROTOCOL_en.md`/`_vn.md` as a new Section 2.1. Confirmed `duplicate_user_item_rows = 0` for `Baby_Products` (previously `UNKNOWN`) and recorded full retention/OOV/singleton-rate figures for the three candidate policies `all_observed`/`P4`/`P5`. No policy was chosen; G2-B remains `OPEN`. Before integrating, flagged and asked the user about a contradiction between `06_code/docs/DATASET_AUDIT_vn.md` (implying the run was already done) and the continuity file/`DATASET_AUDIT_RESULTS_vn.md` (still recording it as open); the user confirmed the run really happened and asked me to pull it from Drive myself.
- **Evidence/source:** `Baby_Products_protocol_audit.json` (Google Drive, `sha256` `e2a8d0498afed767ee2615db7fac549559d82490b1a73c7241b84b5e9e8c279e`, `audit_config_sha256` `f72e6b62ae5210fb96078024177eb7e6d2bd6d9c0737acafcaea41007b1649e1`, `notebook_revision` `self-contained-protocol-audit-v1-2026-09-02`). The candidate `t1`/`t2` cutoffs match the values already recorded in `06_code/docs/DATASET_AUDIT_RESULTS_en.md`.
- **Agents consulted, roles, and identifiers:** Riemann (subagent, this session) independently audited the arithmetic/derived percentages and overclaim risk across 12 numeric claims drawn from the JSON. Poincare (subagent, this session) independently audited gate-rule compliance, in particular G2's non-comparative constraint and the evidence-scope boundary. Each agent received only the spec/JSON from the main agent and did not see the other's answer before responding (independent pass).
- **Independent findings:** Riemann confirmed all 12 numbers match the JSON; found a harmless 1-row discrepancy between `candidate_absolute_split.train_candidate.rows` (4,886,113, pre-quarantine) and `protocol_analysis...train_candidate.rows` (4,886,112, post-quarantine) and recommended always citing the latter when comparing against `clean_deduplicated_events`; recommended also citing `sha256`/`audit_config_sha256`/`notebook_revision`/`retrieved_at` for provenance. Poincare found no overclaiming in the drafted table/next-action prose; required rewording "duplicate = 0" to stay separate from the still-`OPEN` duplicate-row modeling treatment, and required avoiding phrasing like "G2-A nearly done" in favor of listing which fields are `VERIFIED` versus still `PARTIAL`, stating plainly that G2-A is not closed.
- **Cross-critique and adjudication:** Applied both reviewers' corrections in the new Section 2.1: cite the post-quarantine train-candidate row count; leave the `Baby_Products` "Current decision" cell in the portfolio table unchanged; keep "exact duplicate rows VERIFIED 0" separate from "duplicate-row treatment for modeling remains OPEN"; list G2-A's field-level status explicitly (checksum/schema/manifest/`audit_config_sha256` = `VERIFIED`, license/access note = `PARTIAL`) and state plainly that G2-A is not closed rather than "nearly complete."
- **Claims added, verified, contradicted, or retired:** Verified `duplicate_user_item_rows = 0` for `Baby_Products` (retiring the prior `UNKNOWN` status). Added exact retention/OOV/singleton-rate figures for `P4`/`P5`/`all_observed` and the timestamp-tie count (32,050 rows sharing a timestamp value). Added no claim about model quality, novelty, scalability, or a dataset freeze; G2 overall remains `IN_PROGRESS`.
- **What remains uncertain:** The primary interaction policy (`P4`/`P5`/`all_observed`) is not chosen; the duplicate/`0.0` treatment for modeling is not decided; Amazon Reviews'23's `license_or_access_note` remains `PARTIAL`. `06_code/docs/DATASET_AUDIT_RESULTS_en.md`/`_vn.md` (recorded 2026-08-26) have **not** been synchronized with this new JSON — `06_code/docs/DATASET_AUDIT_vn.md` (2026-09-02) had already flagged this sync as needed before this session began; the sync is deferred as a next action because the current task was scoped to updating `DATASET_PORTFOLIO_AND_ANALYSIS_PROTOCOL` only.
- **Next action:** (1) Select the primary interaction policy and the duplicate/`0.0` treatment from pre-registered rationale to close G2-B; (2) synchronize `06_code/docs/DATASET_AUDIT_RESULTS_en.md`/`_vn.md` with the new JSON figures (they still record the Baby duplicate count as `UNKNOWN`, now stale); (3) resolve `license_or_access_note` to close G2-A; (4) once G2-A/G2-B close, build the strict training-only temporal graph to close G2-C.
- **Files created or changed in Do An:** `00_project/DATASET_PORTFOLIO_AND_ANALYSIS_PROTOCOL_en.md`; `00_project/DATASET_PORTFOLIO_AND_ANALYSIS_PROTOCOL_vn.md`; both continuity files (verification register V-003 and this handoff entry).

### Portfolio-batch notebook correction and thesis-result integration — 2026-09-02

- **Decision or result:** Verified the completed persistent `Baby_Products_protocol_audit.json`, corrected its unexpanded source-URL placeholder without changing numerical results, synchronized the living thesis report with the already-registered P4/G2-A/G2-B decision, and expanded the self-contained notebook from one hard-coded category to the three governed Amazon portfolio jobs.
- **Evidence/source:** Drive result ID `1k1o9zrr0wJLdQ5-_ZM_j7k6UrvARMWdV`; SHA-256 `e2a8d0498afed767ee2615db7fac549559d82490b1a73c7241b84b5e9e8c279e`; corrected local machine-readable mirror; the independent semantic/feasibility adjudication already recorded in Section 2.2 of the canonical dataset protocol.
- **Execution design:** `All_Beauty` and `Baby_Products` run full protocol audits. Existing outputs are reused by default, so the completed Baby run is not repeated. `Home_and_Kitchen` runs only a low-memory exact-byte/schema/row-count provenance audit; full scale stress remains gated by G5-S. One portfolio index reconciles job status and outputs.
- **Agents consulted, roles, and identifiers:** No new agents. The research interpretation and P4 decision reuse the prior independent Fisher/Neyman and Riemann/Poincare reviews already recorded; this turn's notebook batching, provenance correction, and artifact synchronization were mechanical implementation work.
- **Claims added, verified, contradicted, or retired:** Added no model, performance, or scalability claim. Verified the complete Baby pre-model audit and preserved the canonical state that P4 is selected and G2-A/G2-B pass for Baby. The URL placeholder was a provenance-metadata defect only; category, raw path, byte count, and checksum already identified the analyzed bytes.
- **What remains uncertain:** Full `All_Beauty` protocol output and `Home_and_Kitchen` provenance output have not yet been produced by the new batch run. Baby G2-C/G2-D remain open.
- **Next action:** Run all cells once in the updated Drive notebook. It should reuse/correct Baby, execute All Beauty, acquire the bounded Home and Kitchen provenance record, and write `dataset_portfolio_audit_index.json`; then integrate those new outputs before G2-C implementation.
- **Files created or changed in Do An:** paired portfolio notebooks; corrected Baby protocol JSON mirror; paired thesis reports; paired dataset protocols and audit docs; both continuity files.

### First portfolio batch execution and Home acquisition correction — 2026-09-02

- **Decision or result:** The user executed the portfolio notebook. Full protocol audits completed for `All_Beauty` and `Baby_Products`; the third job failed before portfolio-index persistence because the extrapolated `datarepo.eng.ucsd.edu` mirror URL returned HTTP 404 for `Home_and_Kitchen`.
- **Evidence/source:** New Drive files `All_Beauty_protocol_audit.json` (ID `1UsUsLtZ7cU4GMOsrG_PzgnRfdp_Loi5W`) and `Baby_Products_protocol_audit.json` (ID `1KV2zNm8X9RR4v6t_TBRTYWw_taETFvX-`); official Amazon Reviews'23 0-core page and its `Home_and_Kitchen` download link, which resolves to the `mcauleylab.ucsd.edu/public_datasets/data/...` host/path.
- **Result interpretation:** The complete All Beauty audit confirms zero duplicate pairs and 448 rows in shared timestamp values. Under all-observed/P4/P5, user singleton rates are 93.22%/94.16%/94.76%, while validation warm-target retention is 4.77%/3.98%/3.24% and test retention is 2.28%/1.91%/1.65%. This validates the protocol on a second category and reinforces its diagnostic-only role; it is not model or scalability evidence.
- **Correction:** The notebook now uses the exact official download host. `Home_and_Kitchen` remains a low-memory provenance/schema/row-count job. Download exceptions are recorded as `DOWNLOAD_FAILED` and no longer prevent `dataset_portfolio_audit_index.json` from being written. Existing All Beauty and Baby outputs are reused on the next run.
- **Agents consulted, roles, and identifiers:** No new agents; this was source-path diagnosis, mechanical error handling, and integration of descriptive results under previously reviewed interpretation rules.
- **What remains uncertain:** Home and Kitchen exact bytes, hash, schema, and row count remain unexecuted. Baby G2-C/G2-D remain open.
- **Next action:** Rerun the final notebook cell. It should reuse the two completed protocol outputs, attempt only the corrected Home acquisition/audit, and persist the portfolio index regardless of download outcome.
- **Files created or changed in Do An:** paired notebooks; paired audit docs; paired thesis reports; All Beauty machine-readable result mirror; both continuity files.

### Amazon portfolio audit index completed — 2026-09-02

- **Decision or result:** Verified the corrected batch rerun and persistent portfolio index. All Beauty and Baby full protocol outputs were reused, and the bounded Home and Kitchen provenance/schema/row-count job executed successfully.
- **Evidence/source:** Drive index ID `1tTrcOjzF2-1mWjwXhQVdKtN2hczAuuJV`; Home result ID `1T9dNLJzCkQETons8PRdJYO__Gitsg1MW`; exact Home source URL, 1,420,416,432 compressed bytes, 66,623,880 rows, four-column schema, and SHA-256 `9be4e2dc8b3dc513c02521644b2ae55f722b2941767e539dcfe518f6bdd4f70b`.
- **Claims added, verified, contradicted, or retired:** Verified Home byte identity, schema, row count, and raw scale (about 11.19× Baby by rows). Added no project-derived Home user/item/degree statistic, semantic finding, feasibility measurement, or scalability claim.
- **Gate effect:** G2 remains `IN_PROGRESS` only for Baby G2-C/G2-D. Home provenance is acquired; conditional full Home scale execution remains G5-S evidence and is not a G2 prerequisite.
- **Agents consulted, roles, and identifiers:** None; this was readback, exact arithmetic, artifact synchronization, and gate-status propagation under existing reviewed rules.
- **Next action:** Implement the strict training-only P4 temporal graph for Baby, reconcile every warm-start/OOV exclusion, and run the bounded G2-D evaluator feasibility path.
- **Files created or changed in Do An:** paired plans, dataset protocols, thesis reports, audit docs, continuity files, and local mirrors of Home provenance plus the portfolio index.

### Baby P4 G2-C/G2-D execution path implemented — 2026-09-02

- **Decision or result:** Implemented a new paired, self-contained Colab notebook for the Baby P4 training-only temporal graph, exact warm-start/OOV accounting, exact-candidate construction, deterministic artifact export, retained-graph statistics, and bounded G2-D catalog traversal. No separate Python file is required on Drive.
- **Evidence/source:** Local notebook JSON and code-cell syntax validation; `test_g2c_notebook.py` toy end-to-end execution; the full local suite passes 13/13. Embedded implementation SHA-256 is `18dd46ad142491891ce6d5b1c4add4a8e6e5953a22c46bd429b3732798117998`.
- **Drive synchronization:** English notebook ID `1rWEcvnXKlnuXpIUvP1WQwdum-yhaMwH4`; Vietnamese notebook ID `1oOfNrqsbITuMjFVysTBK-TbrB8-enqFb`. Both were updated in place in the existing Colab Notebooks folder.
- **Implementation boundary:** Candidate cutoffs remain explicitly reviewable. Mappings and graph statistics use training positives only; validation/test exclusions reconcile by unseen-user/unseen-item reason; prior history is strictly earlier than each target; the bounded traversal trains and scores no model. The manifest records hashes of generated artifacts and the embedded implementation, but intentionally does not contain an impossible self-referential hash of itself.
- **Agents consulted, roles, and identifiers:** None. This was implementation of already reviewed protocol decisions and did not introduce a new scientific adjudication.
- **Claims added, verified, contradicted, or retired:** Verified implementation and toy behavior only. No full Baby graph statistic, cutoff freeze, G2-C/G2-D pass, NDCG/Recall, resource-profile, or scalability claim was added.
- **Next action:** Run all cells in the Vietnamese or English Drive notebook. Read back `MyDrive/Phase2_Amazon_Audit/g2c_baby_p4/baby_p4_g2c_manifest.json` and its five compressed artifacts; then verify invariants/arithmetic and accept or revise the cutoffs before deciding G2-C/G2-D.
- **Files created or changed in Do An:** paired `02_baby_p4_temporal_graph_*` notebooks; `tests/test_g2c_notebook.py`; paired G2-C execution guides; paired plans, thesis reports, traceability files, code READMEs, and continuity files.

### G2 and dataset-role criteria added to the living summary — 2026-09-02

- **Decision or result:** Added an explicit explanation of Dataset Gate G2 and a pre-model classification rule for primary, development/diagnostic, scale-stress, and optional datasets. The canonical protocol and cumulative thesis report now explain why the full current G2-C/G2-D notebook is Baby-only even though the portfolio audit covers All Beauty, Baby, and Home.
- **Classification criteria:** semantic fit; retained graph size/structural difficulty; defensible warm-start coverage; reproducible processing/compute feasibility; and the claim the dataset is intended to support. Roles must be assigned before model scores and may change only through a recorded validity/feasibility review.
- **Current classification:** `Baby_Products` primary; `All_Beauty` diagnostic because its singleton/retention profile makes the primary warm-start estimand too narrow; `Home_and_Kitchen` conditional scale stress because it is about 11.19 times Baby by raw rows and full execution belongs to G5-S.
