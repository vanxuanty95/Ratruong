...
| V-003 | Exact Amazon Reviews'23 release, category và post-filter count cho Phase 2 | `PARTIAL: PERSISTENT PROTOCOL AUDIT ĐÃ CHẠY CHO BABY_PRODUCTS (2026-09-02); POST-FILTER (FROZEN) CHƯA CÓ VÌ PRIMARY POLICY CHƯA CHỌN` | Pre-registered primary interaction policy (P4/P5/all-observed), duplicate/`0.0` treatment và training-only temporal graph theo policy đã chọn | Chọn primary policy từ evidence trong `00_project/DATASET_PORTFOLIO_AND_ANALYSIS_PROTOCOL_vn.md` Mục 2.1, đóng G2-B, rồi xây frozen training graph để đóng G2-C |
...
- Các deliverable luận văn tích lũy đã được khởi tạo: bilingual working thesis report trong `04_thesis`, bilingual PowerPoint deck trong `05_slides` và Python/Colab source có tài liệu song ngữ trong `06_code`. Local suite pass 13/13 pure-Python test, gồm G2-C toy path; điều này không phải validation của full Baby run, phương pháp Phase 2 cuối cùng, full oracle suite hoặc benchmark.
...
- Persistent portfolio audit xác nhận 693,929 valid `All_Beauty` row và 5,953,891 parsed `Baby_Products` row, zero exact duplicate pair ở cả hai và một Baby `rating=0.0` bị quarantine. Baby P4 cùng G2-A/G2-B đã được chọn/pass; provider absolute split không phải primary warm-start protocol.
- Portfolio analyzer cùng cặp notebook G2-C/G2-D nay cover từ provenance tới deterministic training-only graph construction, OOV reconciliation, exact-candidate rule và bounded traversal. Full Baby G2-C/G2-D execution cùng evidence review vẫn mở.
...
### Notebook Colab audit dataset self-contained — 2026-09-02

- **Quyết định hoặc kết quả:** Thay launcher Colab phụ thuộc script ngoài bằng notebook audit dataset self-contained. Mỗi notebook trong cặp nay nhúng toàn bộ analyzer dùng standard library/SQLite, dùng lại hoặc tải raw artifact persistent, chạy toàn bộ protocol audit và ghi JSON persistent mà không cần file `.py` riêng trên Drive.
- **Bằng chứng/nguồn:** Static parse mọi code cell của cả hai notebook; kiểm tra JSON notebook; logic hash nhúng và notebook revision được ghi vào provenance của kết quả; các yêu cầu audit hiện có trong `06_code/docs/DATASET_AUDIT_vn.md` và bản tiếng Anh đồng bộ.
- **Agent được tham vấn, vai trò và định danh:** Không có. Đây là sửa mechanical về packaging và execution path theo yêu cầu trực tiếp của người dùng; không thêm quyết định nghiên cứu hoặc diễn giải khoa học.
- **Claim được thêm, xác minh, bác bỏ hoặc hủy:** Chỉ xác minh artifact notebook local là self-contained và hợp lệ về cú pháp. Thay đổi này chưa tạo full protocol run `Baby_Products`, model result, gate closure hoặc dataset-policy decision.
- **Điều còn chưa chắc chắn:** Kết quả complete persistent Colab run và bước phân xử policy P4/P5/all-observed sau đó vẫn mở.
- **Hành động tiếp theo:** Run all một lần trong notebook Drive tiếng Việt hoặc tiếng Anh, sau đó đọc `Baby_Products_protocol_audit.json` và tích hợp evidence vào thesis report tích lũy trước khi đóng bất kỳ G2 sub-gate nào.
- **File được tạo hoặc sửa trong Do An:** hai notebook `01_amazon_dataset_audit_*`; hai dataset-audit protocol document; hai code README; hai continuity file.


### Tích hợp persistent protocol-audit evidence cho Baby_Products — 2026-09-02

- **Quyết định hoặc kết quả:** Đã truy cập Google Drive riêng của người dùng (theo yêu cầu trực tiếp), trích xuất `Baby_Products_protocol_audit.json` thật (folder `audit_outputs`, `retrieved_at` 2026-09-02T11:49:02Z) và tích hợp vào `00_project/DATASET_PORTFOLIO_AND_ANALYSIS_PROTOCOL_en.md`/`_vn.md` như Mục 2.1 mới. Xác nhận `duplicate_user_item_rows = 0` cho `Baby_Products` (trước đây `UNKNOWN`) và ghi lại retention/OOV/singleton-rate đầy đủ cho ba candidate policy `all_observed`/`P4`/`P5`. Không policy nào được chọn; G2-B vẫn `MỞ`. Trước khi tích hợp, đã phát hiện và hỏi người dùng về một mâu thuẫn giữa `06_code/docs/DATASET_AUDIT_vn.md` (ngụ ý đã chạy xong) và continuity/`DATASET_AUDIT_RESULTS_vn.md` (ghi vẫn còn mở); người dùng xác nhận đã chạy thật và yêu cầu tự lấy từ Drive.
- **Bằng chứng/nguồn:** `Baby_Products_protocol_audit.json` (Google Drive, `sha256` `e2a8d0498afed767ee2615db7fac549559d82490b1a73c7241b84b5e9e8c279e`, `audit_config_sha256` `f72e6b62ae5210fb96078024177eb7e6d2bd6d9c0737acafcaea41007b1649e1`, `notebook_revision` `self-contained-protocol-audit-v1-2026-09-02`). Candidate `t1`/`t2` đối chiếu khớp với giá trị đã ghi trước đó trong `06_code/docs/DATASET_AUDIT_RESULTS_vn.md`.
- **Agent được tham vấn, vai trò và định danh:** Riemann (subagent, phiên này) audit độc lập arithmetic/derived percentage và overclaim risk trên 12 claim số liệu trích từ JSON. Poincare (subagent, phiên này) audit độc lập gate-rule compliance, đặc biệt ràng buộc non-comparative của G2 và ranh giới phạm vi evidence. Mỗi agent chỉ nhận spec/JSON từ main agent, không thấy bài của nhau trước khi trả lời (independent pass).
- **Independent finding:** Riemann xác nhận cả 12 con số khớp JSON; phát hiện discrepancy vô hại 1-row giữa `candidate_absolute_split.train_candidate.rows` (4,886,113, trước quarantine) và `protocol_analysis...train_candidate.rows` (4,886,112, sau quarantine) và đề nghị luôn trích dẫn bản sau khi so sánh với `clean_deduplicated_events`; đề nghị bổ sung citation cho `sha256`/`audit_config_sha256`/`notebook_revision`/`retrieved_at`. Poincare xác nhận không overclaim ở phần mô tả bảng số liệu và next-action; yêu cầu diễn đạt lại "duplicate = 0" tách biệt khỏi duplicate-row treatment cho modeling (vẫn `OPEN`), và yêu cầu không dùng ngôn ngữ kiểu "G2-A gần xong" mà phải liệt kê field nào `VERIFIED` và field nào còn `PARTIAL`, nói rõ gate G2-A chưa đóng.
- **Cross-critique và phân xử:** Áp dụng toàn bộ correction của cả hai reviewer trong Mục 2.1 mới: trích train-candidate row count post-quarantine; giữ nguyên "Quyết định hiện tại" của `Baby_Products` trong bảng portfolio; tách rõ "exact duplicate rows VERIFIED 0" khỏi "duplicate-row treatment cho modeling vẫn OPEN"; liệt kê field-level status của G2-A (checksum/schema/manifest/`audit_config_sha256` = `VERIFIED`, license/access-note = `PARTIAL`) và nói rõ G2-A chưa đóng thay vì "gần hoàn tất".
- **Claim được thêm, verify, contradict hoặc retire:** Verify `duplicate_user_item_rows = 0` cho `Baby_Products` (retire trạng thái `UNKNOWN` trước đó). Thêm exact retention/OOV/singleton-rate cho `P4`/`P5`/`all_observed` và timestamp-tie count (32,050 row chia sẻ timestamp). Không thêm claim về model, novelty, scalability hay dataset freeze; G2 tổng thể vẫn `IN_PROGRESS`.
- **Điều còn chưa chắc chắn:** Primary interaction policy (`P4`/`P5`/`all_observed`) chưa chọn; duplicate/`0.0` treatment cho modeling chưa quyết định; `license_or_access_note` của Amazon Reviews'23 vẫn `PARTIAL`. `06_code/docs/DATASET_AUDIT_RESULTS_en.md`/`_vn.md` (ghi 2026-08-26) **chưa được đồng bộ** với JSON mới này — file `06_code/docs/DATASET_AUDIT_vn.md` (2026-09-02) đã tự nêu yêu cầu sync này trước khi session này bắt đầu; việc sync bị hoãn sang next action vì task hiện tại chỉ giới hạn ở cập nhật `DATASET_PORTFOLIO_AND_ANALYSIS_PROTOCOL`.
- **Hành động tiếp theo:** (1) Chọn primary interaction policy và duplicate/`0.0` treatment bằng pre-registered rationale để đóng G2-B; (2) đồng bộ `06_code/docs/DATASET_AUDIT_RESULTS_en.md`/`_vn.md` với số liệu JSON mới (hiện vẫn ghi duplicate Baby là `UNKNOWN`, đã lỗi thời); (3) giải quyết `license_or_access_note` để đóng G2-A; (4) sau khi G2-A/G2-B đóng, xây strict training-only temporal graph để đóng G2-C.
- **File được tạo hoặc sửa trong Do An:** `00_project/DATASET_PORTFOLIO_AND_ANALYSIS_PROTOCOL_en.md`; `00_project/DATASET_PORTFOLIO_AND_ANALYSIS_PROTOCOL_vn.md`; hai continuity file (verification register V-003 và handoff entry này).

### Sửa notebook batch theo portfolio và tích hợp kết quả vào luận văn — 2026-09-02

- **Quyết định hoặc kết quả:** Đã xác minh persistent `Baby_Products_protocol_audit.json` chạy hoàn tất, sửa source-URL placeholder chưa expand mà không đổi numerical result, đồng bộ thesis report tích lũy với quyết định P4/G2-A/G2-B đã đăng ký, và mở rộng notebook self-contained từ một category hard-code thành ba Amazon portfolio job được quản trị.
- **Bằng chứng/nguồn:** Drive result ID `1k1o9zrr0wJLdQ5-_ZM_j7k6UrvARMWdV`; SHA-256 `e2a8d0498afed767ee2615db7fac549559d82490b1a73c7241b84b5e9e8c279e`; machine-readable mirror local đã sửa; independent semantic/feasibility adjudication đã ghi trong Mục 2.2 của dataset protocol chuẩn.
- **Thiết kế thực thi:** `All_Beauty` và `Baby_Products` chạy full protocol audit. Output đã có được dùng lại mặc định nên complete Baby run không chạy lại. `Home_and_Kitchen` chỉ chạy low-memory exact-byte/schema/row-count provenance audit; full scale stress vẫn bị gate G5-S khóa. Một portfolio index đối soát trạng thái job và output.
- **Agent được tham vấn, vai trò và định danh:** Không có agent mới. Diễn giải nghiên cứu và quyết định P4 tái sử dụng review độc lập Fisher/Neyman cùng Riemann/Poincare đã ghi trước; việc batch notebook, sửa provenance và đồng bộ artifact trong lượt này là implementation mechanical.
- **Claim được thêm, xác minh, bác bỏ hoặc hủy:** Không thêm model, performance hoặc scalability claim. Xác minh complete Baby pre-model audit và giữ trạng thái chuẩn rằng P4 đã chọn, G2-A/G2-B pass cho Baby. URL placeholder chỉ là lỗi provenance metadata; category, raw path, byte count và checksum vốn đã định danh bytes được phân tích.
- **Điều còn chưa chắc chắn:** Full protocol output `All_Beauty` và provenance output `Home_and_Kitchen` chưa được new batch run tạo. G2-C/G2-D của Baby vẫn mở.
- **Hành động tiếp theo:** Run all một lần trong notebook Drive đã cập nhật. Notebook sẽ dùng lại/sửa Baby, execute All Beauty, acquire bounded provenance record cho Home and Kitchen và ghi `dataset_portfolio_audit_index.json`; sau đó tích hợp các output mới trước khi implement G2-C.
- **File được tạo hoặc sửa trong Do An:** cặp portfolio notebook; Baby protocol JSON mirror đã sửa; cặp thesis report; cặp dataset protocol và audit doc; hai continuity file.

### Thực thi portfolio batch lần đầu và sửa acquisition Home — 2026-09-02

- **Quyết định hoặc kết quả:** Người dùng đã chạy portfolio notebook. Full protocol audit hoàn tất cho `All_Beauty` và `Baby_Products`; job thứ ba thất bại trước khi lưu portfolio index vì mirror URL `datarepo.eng.ucsd.edu` được ngoại suy trả HTTP 404 cho `Home_and_Kitchen`.
- **Bằng chứng/nguồn:** Drive file mới `All_Beauty_protocol_audit.json` (ID `1UsUsLtZ7cU4GMOsrG_PzgnRfdp_Loi5W`) và `Baby_Products_protocol_audit.json` (ID `1KV2zNm8X9RR4v6t_TBRTYWw_taETFvX-`); official Amazon Reviews'23 0-core page cùng download link `Home_and_Kitchen`, trỏ tới host/path `mcauleylab.ucsd.edu/public_datasets/data/...`.
- **Diễn giải kết quả:** Complete All Beauty audit xác nhận zero duplicate pair và 448 row trong shared timestamp value. Theo all-observed/P4/P5, user singleton rate là 93.22%/94.16%/94.76%, validation warm-target retention là 4.77%/3.98%/3.24% và test retention là 2.28%/1.91%/1.65%. Kết quả validate protocol trên category thứ hai và củng cố vai trò diagnostic-only; đây không phải model hoặc scalability evidence.
- **Sửa lỗi:** Notebook nay dùng đúng official download host. `Home_and_Kitchen` vẫn là job provenance/schema/row-count ít tốn RAM. Download exception được ghi `DOWNLOAD_FAILED` và không còn ngăn lưu `dataset_portfolio_audit_index.json`. Lần chạy sau dùng lại output All Beauty và Baby hiện có.
- **Agent được tham vấn, vai trò và định danh:** Không có agent mới; đây là chẩn đoán source path, xử lý lỗi mechanical và tích hợp descriptive result theo interpretation rule đã review trước.
- **Điều còn chưa chắc chắn:** Exact bytes, hash, schema và row count Home and Kitchen vẫn chưa execute. G2-C/G2-D của Baby vẫn mở.
- **Hành động tiếp theo:** Chạy lại cell cuối của notebook. Notebook sẽ dùng lại hai protocol output đã hoàn tất, chỉ thử corrected Home acquisition/audit và lưu portfolio index bất kể download thành công hay thất bại.
- **File được tạo hoặc sửa trong Do An:** cặp notebook; cặp audit doc; cặp thesis report; machine-readable result mirror All Beauty; hai continuity file.

### Hoàn tất Amazon portfolio audit index — 2026-09-02

- **Quyết định hoặc kết quả:** Đã xác minh corrected batch rerun cùng persistent portfolio index. Full protocol output All Beauty và Baby được dùng lại; bounded provenance/schema/row-count job Home and Kitchen execute thành công.
- **Bằng chứng/nguồn:** Drive index ID `1tTrcOjzF2-1mWjwXhQVdKtN2hczAuuJV`; Home result ID `1T9dNLJzCkQETons8PRdJYO__Gitsg1MW`; exact Home source URL, 1,420,416,432 compressed bytes, 66,623,880 row, schema bốn cột và SHA-256 `9be4e2dc8b3dc513c02521644b2ae55f722b2941767e539dcfe518f6bdd4f70b`.
- **Claim được thêm, xác minh, bác bỏ hoặc hủy:** Xác minh Home byte identity, schema, row count và raw scale (khoảng 11.19 lần Baby theo số row). Không thêm project-derived Home user/item/degree statistic, semantic finding, feasibility measurement hoặc scalability claim.
- **Ảnh hưởng gate:** G2 vẫn `IN_PROGRESS` chỉ vì Baby G2-C/G2-D. Home provenance đã acquire; conditional full Home scale execution vẫn là evidence G5-S và không phải prerequisite G2.
- **Agent được tham vấn, vai trò và định danh:** Không có; đây là readback, exact arithmetic, đồng bộ artifact và lan truyền gate status theo rule đã review.
- **Hành động tiếp theo:** Implement strict training-only P4 temporal graph cho Baby, đối soát mọi warm-start/OOV exclusion và chạy bounded G2-D evaluator feasibility path.
- **File được tạo hoặc sửa trong Do An:** cặp plan, dataset protocol, thesis report, audit doc, continuity file và local mirror của Home provenance cùng portfolio index.

### Implement path thực thi Baby P4 G2-C/G2-D — 2026-09-02

- **Quyết định hoặc kết quả:** Đã implement cặp notebook Colab self-contained mới cho Baby P4 training-only temporal graph, exact warm-start/OOV accounting, exact-candidate construction, deterministic artifact export, retained-graph statistic và bounded G2-D catalog traversal. Drive không cần file Python riêng.
- **Bằng chứng/nguồn:** Validate local notebook JSON và code-cell syntax; execute toy end-to-end qua `test_g2c_notebook.py`; toàn bộ local suite pass 13/13. Embedded implementation SHA-256 là `18dd46ad142491891ce6d5b1c4add4a8e6e5953a22c46bd429b3732798117998`.
- **Đồng bộ Drive:** Notebook tiếng Anh ID `1rWEcvnXKlnuXpIUvP1WQwdum-yhaMwH4`; notebook tiếng Việt ID `1oOfNrqsbITuMjFVysTBK-TbrB8-enqFb`. Cả hai được update tại chỗ trong thư mục Colab Notebooks hiện có.
- **Ranh giới implementation:** Candidate cutoff vẫn được ghi rõ là chờ review. Mapping và graph statistic chỉ dùng training positive; validation/test exclusion đối soát theo lý do unseen-user/unseen-item; prior history phải nhỏ hơn nghiêm ngặt timestamp từng target; bounded traversal không train hoặc score model. Manifest ghi hash của generated artifact và embedded implementation, nhưng cố ý không chứa self-referential hash bất khả thi của chính nó.
- **Agent được tham vấn, vai trò và định danh:** Không có. Đây là implementation của protocol decision đã review, không phải scientific adjudication mới.
- **Claim được thêm, xác minh, bác bỏ hoặc hủy:** Chỉ verify implementation và toy behavior. Không thêm full Baby graph statistic, cutoff freeze, G2-C/G2-D pass, NDCG/Recall, resource profile hoặc scalability claim.
- **Hành động tiếp theo:** Run all trong notebook Drive tiếng Việt hoặc tiếng Anh. Đọc lại `MyDrive/Phase2_Amazon_Audit/g2c_baby_p4/baby_p4_g2c_manifest.json` cùng năm compressed artifact; sau đó verify invariant/arithmetic và chấp nhận hoặc sửa cutoff trước khi quyết định G2-C/G2-D.
- **File được tạo hoặc sửa trong Do An:** cặp notebook `02_baby_p4_temporal_graph_*`; `tests/test_g2c_notebook.py`; cặp hướng dẫn thực thi G2-C; cặp plan, thesis report, traceability, code README và continuity file.

### Bổ sung G2 và tiêu chí phân loại dataset vào living summary — 2026-09-02

- **Quyết định hoặc kết quả:** Đã bổ sung giải thích tường minh về Dataset Gate G2 và quy tắc pre-model để phân loại primary, development/diagnostic, scale-stress và optional dataset. Protocol chuẩn cùng cumulative thesis report nay giải thích vì sao full G2-C/G2-D notebook hiện chỉ có Baby dù portfolio audit bao phủ All Beauty, Baby và Home.
- **Tiêu chí phân loại:** semantic fit; retained graph size/structural difficulty; warm-start coverage có thể bảo vệ; processing/compute khả thi và tái lập; cùng claim mà dataset được dùng để hỗ trợ. Vai trò phải được gán trước model score và chỉ đổi qua validity/feasibility review có record.
- **Phân loại hiện tại:** `Baby_Products` là primary; `All_Beauty` là diagnostic vì singleton/retention profile làm primary warm-start estimand quá hẹp; `Home_and_Kitchen` là conditional scale stress vì raw row khoảng 11.19 lần Baby và full execution thuộc G5-S.
