...
\usepackage{graphicx}
\graphicspath{{img/}{../06_code/results/data_story/}{../06_code/results/dataset_deep_analysis/figures/}}
\usepackage{xcolor}
\usepackage{enumitem}
...
2 Dữ liệu & Chọn dataset, chất lượng, rating, histogram, long-tail, cold-start, thời gian, graph & Một phần & Notebook 11 \\
3 Góp ý giảng viên & Trả lời từng câu hỏi ngày 3/9 & Có; số ngữ nghĩa chờ & Notebook 11, R5 \\
...
\pending{Notebook 11 (S4)}{Kích thước lân cận 1--3 bước thực tế của user và tỷ lệ graph bị chạm theo batch 1.024--65.536 triplet.}
...
Bot, spam, đánh giá dồn dập & Chưa kiểm tra & Chỉ báo cáo & Không xóa khi chưa có quy tắc đăng ký trước \\
...
\pending{Notebook 11 (S3)}{Số user đánh giá $\ge$10, $\ge$20, $\ge$50 sản phẩm trong một ngày và tỷ lệ tương tác của họ; nhóm cùng timestamp của một user; sản phẩm tăng đột biến theo ngày; user có $\ge$10 rating giống hệt nhau.}
...
\pending{Notebook 11 (S1)}{Rating trung bình, tỷ lệ 5\rstar{} và $\le$3\rstar{} theo độ phổ biến sản phẩm và theo mức hoạt động của user; tỷ lệ user chỉ cho 5\rstar{}; độ lệch rating mỗi user.}
...
Dữ liệu trải từ 10/06/2000 đến 12/09/2023, tăng mạnh từ 2013, đỉnh 2019--2020 rồi giảm. Phân bố thay đổi theo thời gian nên đồ án chia theo thời gian thay vì ngẫu nhiên: $t_1$ = 11/08/2021, $t_2$ = 16/07/2022. Cửa sổ development $[t_0, t_1)$ với $t_0$ = 05/09/2020 ước tính khoảng 546 nghìn tương tác P4 (khoảng 14\% trước $t_1$), nên quy tắc $t_0$ chính dự kiến đủ điều kiện.

\pending{Notebook 11 (S2, R1)}{User mới và sản phẩm mới theo năm; khoảng cách giữa hai tương tác của cùng user; thời gian sống của sản phẩm; độ trôi của top 1\% sản phẩm giữa các năm; số liệu chính xác của development split.}
...
\pending{Notebook 11 (S4, R1)}{Số cặp sản phẩm đồng mua; tỷ lệ sản phẩm không có sản phẩm đồng mua nào (không nhận được collaborative signal qua 2 bước); sản phẩm chỉ được user một-tương-tác chạm tới; target development nằm trong 3 bước của user bao nhiêu phần trăm, theo nhóm head/body/tail.}
...
\pending{Notebook 11 (S6)}{Độ phủ title/danh mục/giá/store trên 162.125 sản phẩm train; độ sâu cây danh mục; top danh mục cấp 2 theo tỷ lệ sản phẩm và tỷ lệ tương tác; giá trung vị theo head/body/tail.}
...
Có metadata sản phẩm & Dùng cho đánh giá ngữ nghĩa, không huấn luyện \\
\bottomrule
\end{tabularx}
\end{table}
...
\pending{Notebook 11 (S6), R5}{Độ phủ metadata; giá trị semantic match@20 và ILD@20 cho từng phương pháp.}
...
\pending{Notebook 11 (S4, R1), R6}{Tỷ lệ sản phẩm không có đồng mua; target development trong 3 bước; phân phối node được sampler chọn.}
...
16--22/9 & R1 & Notebook 11: phân tích sâu + development split; hoàn tất R2 & Đang làm \\
