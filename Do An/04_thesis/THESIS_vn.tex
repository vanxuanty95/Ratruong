...
\begin{itemize}
\item \textbf{0.5 (17/09/2026)} Rà soát chống bịa đặt: sửa diễn giải Jaccard, dải \texttt{loss\_coef} của GRAPES, tỷ lệ user mới 54--60\%; bỏ các suy đoán không có số liệu (nguyên nhân bất thường, thay backbone, ví dụ distributed). Số phái sinh ghi tại \texttt{results/doc\_facts/doc\_facts.json}; kết quả oracle R2 và mutation ghi tại \texttt{results/r2\_oracle\_verification/}.
...
Distributed training & Chia dữ liệu/model cho nhiều GPU hoặc nhiều máy & Cần nhiều thiết bị; bản thân nó không giảm lượng tính của một batch & Ngoài phạm vi \\
Chia đồ thị & Cluster-GCN \cite{clustergcn}: lấy khối node của subgraph dày tìm bằng clustering, chỉ tìm hàng xóm trong subgraph & Cạnh ra ngoài subgraph không được dùng trong batch & Không chọn \\
Embedding lịch sử & GAS \cite{gas}: cắt nhánh computation graph bằng embedding từ vòng lặp trước, có cận sai số & Lưu embedding lịch sử cho mọi node (2,48 triệu) mỗi layer; sai số xấp xỉ & Không chọn \\
...
Hai hướng không loại trừ nhau: distributed tăng phần cứng, sampling giảm lượng tính mỗi batch. Đồ án chọn sampling vì đề tài hướng tới và giới hạn một GPU T4.
...
\begin{table}[h]
\centering\small
\caption{Tái hiện GRAPES ở giai đoạn 1 trên Tesla T4 (accuracy trung bình; nguồn \texttt{results/reference\_sources/phase1\_grapes\_results.json}).}\label{tab:phase1}
\begin{tabular}{lcccc}
\toprule
...
\end{tabularx}
\end{table}
User-ngày có $\ge$10 đánh giá chiếm 0,87\% tương tác, $\ge$20 chiếm 0,10\%. Nguyên nhân (mua sắm theo đợt, đánh giá hàng loạt, khuyến mãi) chưa được xác định. Đồ án không loại dòng nào và ghi nhận như một hạn chế.
...
Coi mọi rating là positive sẽ gộp cả 1--3\rstar{} vào positive; chỉ lấy 5\rstar{} làm mất 14,65\% tương tác và tăng cold-start. P4 là điểm cân bằng và được chọn trước khi có kết quả model.
...
\end{tabular}
\end{table}
Sản phẩm phổ biến vừa nhiều tương tác vừa được đánh giá tốt hơn, nên tương tác positive P4 còn dồn về head mạnh hơn tương tác thô. Phía user: rating trung bình tăng từ 4,18 (user 1 rating) lên 4,34 (user $>$20 rating); 44,2\% user có $\ge$2 rating cho mọi sản phẩm cùng một mức; 18,1\% user có $\ge$5 rating chỉ cho 5\rstar{}; độ lệch chuẩn trung vị 0,58. Việc nhiều user cho mọi sản phẩm cùng một mức gợi ý giá trị rating tuyệt đối khó so sánh giữa user (diễn giải, chưa kiểm định); đồ án dùng tín hiệu nhị phân 4--5\rstar{}.
...
\end{figure}
Mỗi năm từ 2014 đến 2023, 54--60\% rating đến từ user lần đầu xuất hiện. Khoảng cách trung vị giữa hai tương tác của cùng user là 20,9 ngày (p90: 598 ngày), và 41,4\% cặp tương tác liên tiếp nằm trong cùng một ngày: user thường đánh giá theo đợt. Sản phẩm có $\ge$2 rating hoạt động trung vị 565 ngày. Top 1\% sản phẩm phổ biến chỉ trùng một phần giữa hai năm liên tiếp: Jaccard 0,45--0,54 giai đoạn 2015--2021 (nếu hai năm có cùng số sản phẩm top 1\%, tương đương 62--70\% sản phẩm top năm sau đã có trong top năm trước). Tỷ lệ rating vào top 1\% của năm trước là 23,8--30,3\%, thấp hơn 30,8--33,6\% vào top 1\% của chính năm đó. Rating trung bình theo năm đạt 4,35 năm 2019 rồi giảm còn 4,06 năm 2022. Hệ quả: chia theo thời gian là cần thiết; giả thuyết cần kiểm tra là sampler dựa trên bậc tích lũy (M1) bám vào sản phẩm phổ biến cũ.
...
\end{tabular}
\end{table}
Chỉ 29,0\% target có đường đi độ dài 3 từ user tới target trong graph train, tức có một user khác đã mua cả sản phẩm của user lẫn target. Với tail con số là 4,5\%. Điểm số LightGCN là tích vô hướng giữa hai embedding đã lan truyền nên vẫn có thể xếp hạng target xa hơn, nhưng bằng chứng collaborative trực tiếp là rất hạn chế. 46,5\% target thuộc user chỉ có một tương tác trong $G_{\text{dev\_train}}$. Quyết định (DL-003): mọi kết quả holdout được báo thêm theo nhóm có/không có đường độ dài $\le$3.
...
\end{figure}
Metadata phủ 100\% sản phẩm train. Độ phủ theo trường: title 99,99\%, store 98,5\%, danh mục 93,4\%, features 77,6\%, mô tả 62,3\%, giá chỉ 22,6\%. Cây danh mục chủ yếu sâu 4--5 cấp, gồm 96 danh mục cấp 2 và 448 danh mục lá; chỉ 61\% sản phẩm có \texttt{main\_category} là ``Baby'' (tiếp theo là Amazon Home 13,3\%, Health \& Personal Care 4,7\%). Phân bổ theo danh mục cũng lệch: Nursery chiếm 32,7\% sản phẩm nhưng 22,4\% tương tác, Safety 6,0\% sản phẩm nhưng 11,3\% tương tác, Feeding 13,5\% và 17,9\%. Trên 22,6\% sản phẩm có giá, giá trung vị gần như bằng nhau ở head (20,04), body (18,99) và tail (19,47). Kết luận: title và danh mục đủ để đo semantic match và ILD; không dùng giá.
...
Top 1\% sản phẩm chỉ trùng một phần giữa các năm (Jaccard 0,45--0,54) & Theo dõi M1 bám sản phẩm cũ \\
...
\textbf{Coverage@20} là tỷ lệ sản phẩm trong catalog xuất hiện ít nhất một lần trong top-20 của mọi user; đo độ phủ, chưa đo ý nghĩa. \textbf{Conversion} là tỷ lệ gợi ý dẫn tới mua; cần log hiển thị--click--mua và A/B test online, dữ liệu review offline không có. Recall@20 chỉ đo sản phẩm tương lai có vào top-20, không thay được conversion. Ví dụ baseline sanity cũ: MostPop chỉ gợi ý 25 sản phẩm (coverage 0,00015) trong khi BPR-MF đạt 0,0335.
...
\toprule
Tối ưu & Có thể xảy ra & Bằng chứng trên Baby & Cách đo \\
\midrule
...
Chi phí thấp (budget nhỏ) & Giả thuyết: ít ngữ cảnh, chất lượng giảm & Chưa đo (RQ4) & NDCG--thời gian \\
Đưa tail lên & Giả thuyết: exposure tăng, hit không tăng & Chưa đo & Exposure và hit tail \\
Khớp ngữ nghĩa & Giả thuyết: giảm đa dạng & Chưa đo (metadata đã có) & Semantic match + ILD \\
\bottomrule
\end{tabularx}
\end{table}
MostPop (không huấn luyện) và BPR-MF (5 epoch) là baseline sanity cũ trên validation, chỉ minh họa trade-off. Kết quả cuối được trình bày như mặt trận Pareto: một phương pháp không được coi là tốt hơn nếu thua ở một trục mà không thắng ở trục khác.
...
\item \textbf{qua sản phẩm head}: 1\% sản phẩm giữ 44\% cạnh; 53\% target development head có đường độ dài 3.
\item \textbf{qua sản phẩm tail}: ít quan sát (80\% sản phẩm, 10\% cạnh); vai trò là giả thuyết.
\item \textbf{qua user hoạt động nhiều}: cầu nối nhiều sản phẩm; vai trò là giả thuyết.
\end{itemize}
...
Hình~\ref{fig:arch} là khung kiến trúc chung. Sampler và recommender có tham số, optimizer và gradient tách biệt. Sampler chỉ dùng lúc huấn luyện; đánh giá dùng LightGCN full-graph trên embedding đã học. Việc thay backbone khác chưa được thiết kế hay kiểm chứng.
...
\item[R-1 Quá ít phần thưởng.] Budget pilot là batch 65.536 trong 5 epoch, khoảng 300 bước tối ưu; một sampler học được dùng budget này chỉ nhận khoảng 300 phần thưởng vô hướng, mỗi phần thưởng cho trung bình 132.749 node mục tiêu (đo ở notebook 11). Các file config GRAPES dùng batch 256 (mặc định trong main.py là 512). Batch và budget sẽ được chọn lại trên development split, chung cho mọi sampler.
\item[R-2 Thang phần thưởng.] BPR khởi đầu khoảng 0,69 và chênh lệch giữa trajectory rất nhỏ; nếu $\alpha$ quá nhỏ, $\log q$ lấn át phần thưởng. Các file config GRAPES (snapshot local) dùng hệ số 151,6--789.615 cho GFlowNet và 210,5--93.660 cho RL (\texttt{results/reference\_sources/grapes\_official\_configs.json}). $\alpha$ và $\log Z$ khởi đầu được quét trên $D_{\text{dev}}$.
